...
| HDR and DRM | Pass through on the system path; protected buffers work | Tone-mapped through your window; protected content is refused |
...
A full-screen player with controls beside the video wants `Surface`, and so does picture-in-picture, TV and anything HDR. A video in a scrolling list, a video that shrinks into a corner, or a player with a frosted panel over the picture wants `Texture`.
...
`MpvSurface` uses Compose Foundation's own surface composables, so there is no `AndroidView` in between: `SurfaceType.Surface` is `AndroidExternalSurface`, `SurfaceType.Texture` is `AndroidEmbeddedExternalSurface`. `zOrder` and `isOpaque` apply to the first only, because a TextureView has neither.
...
`MpvView` and `MpvSurface` both do all three, through the same code. The sample shows the difference between the two types on a device: the panel at the bottom of each screen is blurred, and the video shows through it only in `Texture` mode.
