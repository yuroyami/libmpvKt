...
loadarch () {
	...
	# The lowest API level the libraries run on. BuildInfo.MIN_SDK and the AAR's minSdk say the same.
	local apilvl=21
	...
	# 16 KB page alignment on every shared library, as Google's page-size guidance asks.
	export LDFLAGS="-Wl,-O1,--icf=safe -Wl,-z,max-page-size=16384 -Wl,-z,common-page-size=16384"
	export AR=llvm-ar
...
build () {
	if [ $1 != "jni" ] && [ ! -d deps/$1 ]; then
		printf >&2 '\e[1;31m%s\e[m\n' "Target $1 not found. Run ./download-deps.sh first."
		return 1
	fi
	if [ $nodeps -eq 0 ]; then
	...
	if [ "$1" == "jni" ]; then
		pushd ..
		BUILDSCRIPT=buildscripts/scripts/jni.sh
	else
		pushd deps/$1
...
usage () {
	printf '%s\n' \
		...
		"Builds the specified target (default: $target) and its dependencies" \
		"-n             Do not build dependencies" \
		"--deps-only    Build everything mpv needs, then stop (CI caches this)" \
...
if [ $depsonly -eq 1 ]; then
	for dep in ${dep_mpv[@]}; do
		build $dep
	done
	exit 0
fi
